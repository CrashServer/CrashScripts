...
for seq_num in $(seq 1 $NUM_SEQUENCES); do
    ...
    # We'll create multiple density layers
    # Layer 1: Ambient bed (all files contributing small pieces)
    # Layer 2: Rhythmic elements 
    # Layer 3: Melodic/prominent features
    ...
    for i in "${!audio_files[@]}"; do
        ...
        for seg in $(seq 1 $num_segments); do
            # Random segment duration (0.5 to 5 seconds)
            seg_duration=$(echo "scale=1; 0.5 + $RANDOM/32768*4.5" | bc)
            ...
            if [ -f "$seg_file" ]; then
                # Random volume (0.1 to 0.8)
                volume=$(echo "scale=2; 0.1 + $RANDOM/32768*0.7" | bc)
                ...
                position=$(echo "scale=1; $RANDOM/32768*28" | bc)
